...
exports.getAllStockData = async(req, res) => {
    ...
    const stockDataPromises = Object.entries(stockMap).map(async([symbol, stock]) => {
        try {
            ...
            const peRatio = await fetchPERatio(symbol); //PeRatioyahooServices 
            const latestEarnings = await fetchLatestEarnings(symbol); //lstEarning <- yahooServices 
    ...
    try {
        // promise for promising to get all the data
